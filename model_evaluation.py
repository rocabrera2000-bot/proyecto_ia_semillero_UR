"""
Model Evaluation Pipeline for Hemochromatosis Synthetic Dataset

Compares classical statistical models vs ML models:
  1. Logistic Regression (with feature ranking)
  2. Random Forest
  3. Support Vector Machine (RBF kernel)
  4. XGBoost
  5. Neural Network (MLP)

Reports AUC, accuracy, sensitivity, specificity, F1 for each.
Uses 5-fold stratified cross-validation with proper imputation/scaling.
"""
...
df = pd.read_csv("hemochromatosis_synthetic_dataset.csv")
...
# Logistic regression coefficients (L2, all features)
lr_full = LogisticRegression(max_iter=2000, random_state=SEED, penalty="l2", C=1.0)
lr_full.fit(X_scaled, y)
coef_abs = np.abs(lr_full.coef_[0])
coef_df = pd.DataFrame({
    "feature": feature_names,
    "LR_coef": lr_full.coef_[0],
    "|LR_coef|": coef_abs,
})
coef_df = coef_df.sort_values("|LR_coef|", ascending=False).reset_index(drop=True)
...
print("\nTop 15 features (by average of MI rank + |LR coefficient| rank):\n")
print(f"{'Rank':>4s}  {'Feature':>28s}  {'MI':>6s}  {'MI_rk':>5s}  "
      f"{'LR_coef':>8s}  {'LR_rk':>5s}  {'Avg_rk':>6s}")
print("-" * 70)
for i, row in rank_df.head(15).iterrows():
    print(f"{i+1:4d}  {row['feature']:>28s}  {row['MI']:6.3f}  "
          f"{int(row['MI_rank']):5d}  {row['LR_coef']:8.3f}  "
          f"{int(row['LR_rank']):5d}  {row['avg_rank']:6.1f}")

# Select top features for the "best-ranked" logistic regression
...
# Print results table
print(f"\n{'Model':>30s} | {'AUC':>12s} | {'Accuracy':>12s} | "
...
for name, res in results.items():
    ...
    f1_m, f1_s = res["F1"]
    print(f"{name:>30s} | {auc_m:.3f}±{auc_s:.3f} | {acc_m:.3f}±{acc_s:.3f} | "
...
best_name = max(results, key=lambda k: results[k]["AUC"][0])
best_auc = results[best_name]["AUC"][0]

lr_top10_auc = results["Logistic Regression (top 10)"]["AUC"][0]
lr_all_auc = results["Logistic Regression (all)"]["AUC"][0]

print(f"\nBest model by AUC: {best_name} (AUC = {best_auc:.4f})")
print(f"\nLogistic Regression (top 10 features) AUC: {lr_top10_auc:.4f}")
print(f"Logistic Regression (all features) AUC:    {lr_all_auc:.4f}")
print(f"Best ML model AUC:                         {best_auc:.4f}")

delta = best_auc - lr_all_auc
if delta > 0.005:
    print(f"\n→ ML advantage over full logistic regression: +{delta:.4f} AUC")
    print(f"  {best_name} outperforms logistic regression, suggesting")
...
rf_pipe.fit(SimpleImputer(strategy="median").fit_transform(X), y)
...
print(f"\n{'Rank':>4s}  {'Feature':>28s}  {'Importance':>10s}")
print("-" * 48)
for i, row in imp_df.head(15).iterrows():
    bar = "█" * int(row["importance"] * 200)
    print(f"{i+1:4d}  {row['feature']:>28s}  {row['importance']:10.4f}  {bar}")
